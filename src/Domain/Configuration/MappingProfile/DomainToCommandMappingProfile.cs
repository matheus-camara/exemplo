...
public class DomainToCommandMappingProfile : Profile
{
    ...
    {
        CreateMap<ICollection<Project>, ICollection<string>>()
            .ConvertUsing(x => x.Select(p => p.Name).ToList());

        CreateMap<Project?, string?>()
        ...
        CreateMap<Eligibility, CalculateEligibilityForProCommandResult>()
            ...
            .ForMember(x => x.SelectedProject, x => x.MapFrom(v => v.SelectedProject.Name));
    }
}